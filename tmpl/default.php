...
$doc->addStyleDeclaration('
    ...
    .video-' . $module->id . ' { position: relative; cursor: pointer; /*padding: 8px;*/ box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
    .video-' . $module->id . ' img { width: 100%; display: block; }
    ...
    .overlay-' . $module->id . ' { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.8); z-index: 1000; display: none; }
    .overlay-content-' . $module->id . ' { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 80%; height: 80%; }
...
$doc->addScriptDeclaration('
    document.addEventListener("DOMContentLoaded", function() {
        const videos = document.querySelectorAll(".video-' . $module->id . '");
        videos.forEach(video => {
            video.addEventListener("click", function() {
                ...
                document.querySelector(".overlay-content-' . $module->id . '").innerHTML = "";
                document.querySelector(".overlay-content-' . $module->id . '").appendChild(iframe);
                document.querySelector(".overlay-' . $module->id . '").style.display = "flex";
            });
        });
        document.querySelector(".overlay-' . $module->id .'").addEventListener("click", function(e) {
            if (e.target === this) {
                this.style.display = "none";
                this.querySelector(".overlay-content-' . $module->id . '").innerHTML = "";
            }
        });
    });
');
...
<div class="relative w-full h-auto p-4 lg:p-12 bg-light-cream">
    <div class="video-grid grid w-full grid-cols-1 gap-8 mt-12 md:grid-cols-2 lg:grid-cols-3">
        <?php foreach ($videos['items'] as $video) : ?>
            <div class="video-<?php echo $module->id; ?> flex flex-col w-full h-auto rounded-15px virtue-shadow animation-hover" data-video-id="<?php echo htmlspecialchars($video['id']); ?>">
...
<div id="overlay-<?php echo $module->id; ?>" class="overlay-<?php echo $module->id; ?>">
    <div class="overlay-content-<?php echo $module->id; ?>"></div>
</div>